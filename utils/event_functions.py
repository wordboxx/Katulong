...
async def mark_countdown(ctx):
	print(f"Marks an event to countdown each day.")

	# Load events from events json into "events" variable.
	with open(EVENTS_FILEPATH, 'r') as f:
		events = json.load(f)

	# If there are no events.
	if len(events) == 0:
		await ctx.send("No events to mark! Aborting.")
		return None

	# Lists all events for user to see.
	list_events()

	# Get event from user
	while True:
		try:
			await ctx.send("Which event would you like to mark for countdown? Enter the number:")
			selected_event_input = await ctx.bot.wait_for('message', check=lambda message: message.author == ctx.author)
			selected_event = int(selected_event_input.content)
			break
		except ValueError:
			await ctx.send("Invalid input; please enter an integer.")
		#TODO: FINISH this
